...
const (
	...
	numrangeerr     = "only integer numbers from 1 to 10 allowed"
...
func main() {
	...
	for {
		...
		if x[0] < 48 || x[0] > 57 { //если первый символ первого числа не является цифрой (проверяем по ASCII)
			...
			xint, _ = strconv.Atoi(x) //если же первый символ это цифра, переводим строку в число и записываем в переменную xint
		}
...
func inttorom(num int) string { //это функция-переводчик для перевода int чисел до 100 (int) в римские (string)
	...
	if num == 100 { //100 это исключение, поэтому переводим его буквально
		...
		rom = "XC"
		num = num - 90
	} else if num >= 50 { //если число больше 50, записываем в rom значение "L", а оставшиеся единицы сохраняем в num
		rom = "L"
		num -= 50
		for num >= 10 {
			rom += "X"
		...
		rom = "XL"
		num -= 40
	} else if num >= 10 { //если число больше или равно 10
		for num >= 10 {
...
func inttoromless10(num int) string { //это функция-переводчик для перевода int цифр (int) в римские цифры (string)
	rom := ""
	if num == 9 { //она буквально берёт каждую цифру и переводит в римскую строку
		rom = "IX"
	} else if num == 8 {
		rom = "VIII"
	} else if num == 7 {
